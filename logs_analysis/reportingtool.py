...
popularArticlesSQL = (" SELECT a.title, maxes.num "
            " FROM articles a"
                  " JOIN "
...
popularAuthorsSQL = ("SELECT distinct b.name, sum(maxes.num) "
              " FROM articles a "
              " JOIN "
              " (SELECT path, count(*) as num "
              " FROM log GROUP BY path) "
...
errorRequestsSQL = ("SELECT FILTER.day, FILTER.pct "
                " FROM "
                " (SELECT ALL_RQ.day AS day, 100.0 * BAD_RQ.num / ALL_RQ.num AS pct "
                    " FROM "
...
for row in results:
    print( str(row[0].strftime('%b %d, %Y')) +  ' - ' + str("{0:.2f}".format(round(row[1],2))) + '% errors' )
